...
namespace MaxVideoClub
{
    public partial class frmPartners : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (vRec == false)
            {
                if (validar.IsMatch(txtNombre.Text) && validar.IsMatch(txtApellido.Text) && validar2.IsMatch(txtTelefono.Text) && validar2.IsMatch(txtEdad.Text) && validar.IsMatch(txtEmail.Text) && validar2.IsMatch(txtIfe.Text))
                {
                    if (Cclients.ClienteExistente(txtIfe.Text) == 0)
                    {
                    ...
                    {
                        MessageBox.Show("Imposible, el IFE ya existe en el sistema.");
                    }
                }
                else
                {
        ...
        private void txtFiltro_TextChanged(object sender, EventArgs e)
        {
            ...
            else if (cmbFiltro.Text == "Folio_IFE")
            {

                ((DataTable)dgvParthers.DataSource).DefaultView.RowFilter = string.Format("Convert([Folio_IFE], System.String) like '%{0}%'", txtFiltro.Text.Trim());

            }
            else if (cmbFiltro.Text == "Cualquiera...")
            {

                ((DataTable)dgvParthers.DataSource).DefaultView.RowFilter = string.Format("Convert([Edad], System.String) LIKE '%{0}%' OR [Nombre] LIKE '%{0}%' OR [Apellido] LIKE '%{0}%' OR Convert([Folio_IFE], System.String) LIKE '%{0}%' OR [Email] LIKE '%{0}%' OR Convert([NumDeCliente], System.String) LIKE '%{0}%' OR Convert([Telefono], System.String) LIKE '%{0}%' OR Convert([Folio_IFE], System.String) LIKE '%{0}%'", txtFiltro.Text.Trim());
